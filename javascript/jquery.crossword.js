	...
	$.fn.crossword = function(entryData) {
			...
			*/
			...
			var tbl = ['<table id="puzzle">'],
				...
				rows = [],
				cols = [],
				solved = ($('.puzzle-solved').val() ? $('.puzzle-solved').val().split(',') : []),
				tabindex,
				$actives,
				activePosition = 0,
				activeClueIndex = 0,
				...
				*/
				buildEntries: function() {
					...
					for (var i = 0; i < puzz.data.length; ++i) {
                        // No need to mark the same position twice.
                        if (puzz.data[i].position == currentPosition) {
                            offset++;
                            continue;
                        }

                        // We need to mark position here.
                        var entryselector = '.entry-' + (i + 1 - offset);
                        $groupedLights = $(entryselector);
                        if (!$(entryselector + ':eq(0) span').length){
                            $groupedLights.eq(0)
                                .append('<span>' + puzz.data[i].position + '</span>');
                        }
                        currentPosition = puzz.data[i].position;
					}
