...
class Cross {
    ...
    function savepuzzle( &$crossm, &$crossd, $ctries, $time) {
        ...
        for($i = 0; $i < count($this->m_best_cross_pos); $i++)
        {
            ...
            // Generate key which represents the numerical order of the entries.
            // 3 digits row + 3 digits col + orientation.
            // Sorting the keys will order them top-bottom left-right.
            // There can be both horizontal and vertical for each row-col pair.
            $keyy = str_pad($rec->row, 3, '0', STR_PAD_LEFT);
            $keyx = str_pad($rec->col, 3, '0', STR_PAD_LEFT);
            $key = $keyy. $keyx. (int) !$rec->horizontal;
